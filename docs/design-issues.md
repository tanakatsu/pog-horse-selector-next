...
layout.tsx:27 - スキップリンク欠落。スクリーンリーダーがナビをバイパスできない
...
```
layout.tsx:38 - <main> に id="main-content" 欠落（スキップリンクの飛び先）
layout.tsx:38 - sticky header に env(safe-area-inset-top) なし（ノッチ端末で AppBar が欠ける）
...
login/page.tsx:86 - 'ログイン中...' → 'ログイン中…'（全角省略記号）
```
...
forgot-password/page.tsx:86 - '送信中...' → '送信中…'
```
...
reset-password/page.tsx:82 - '更新中...' → '更新中…'
```
...
```
button.tsx:9 - transition-all → 個別プロパティ列挙（例: transition-[colors,shadow]）
...
```
AppBar.tsx:33 - sticky header に safe-area-inset 対応なし（ノッチ端末）
...
```
HorseSearchInput.tsx:80 - placeholder="母馬名で検索..." → "母馬名で検索…"
...
```
OwnerList.tsx:13 - '読み込み中...' → '読み込み中…'、role="status" 欠落
...
HorseRegisterDialog.tsx:210     - '登録中...' → '登録中…'
...
```
OwnerTable.tsx:52-67 - Pencil / Trash2 アイコンに aria-hidden="true" 欠落
...
```
HorseTable.tsx:64  - placeholder="馬名・父・母で検索" → "馬名・父・母で検索…"
...
HorseTable.tsx:100,108 - Pencil / Trash2 に aria-hidden="true" 欠落（OwnerTable と同様）
