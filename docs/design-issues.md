...
layout.tsx:26 - [x] <meta name="theme-color"> 欠落（背景色と一致させる）
...
AppBar.tsx:39 - [x] APP_TITLE リンクに focus-visible スタイル明示なし（global outline-ring/50 頼り、視認性が低い場合あり）
AppBar.tsx:42 - [x] <nav> に aria-label 欠落（例: aria-label="メインナビゲーション"）
```
...
HorseSearchInput.tsx:78 - [x] Command コンポーネント全体に role="combobox" 相当の ARIA が整備されているか確認
...
OwnerList.tsx:29 - [x] owner.name がリンクテキストだが truncate なし → 超長名でレイアウト崩れ
...
HorseRegisterDialog.tsx:189     - [x] RadioGroup の各行 <div className="flex items-center gap-2">
...
```
OwnerFormDialog.tsx:97  - [x] name Input: autocomplete="off" 欠落（パスワードマネージャーの誤トリガー防止）
OwnerFormDialog.tsx:110 - [x] number Input: inputmode="numeric" 欠落（モバイルで数字キーが出ない）
...
HorseTable.tsx:27-28 - [x] query・page が useState → URL 非同期；ページリロードや共有でリセットされる
                       nuqs 等でクエリパラメータ同期を検討（filter=xxx&page=2）
HorseTable.tsx:93-96 - [x] TableCell で horse.name / sire / mare に truncate なし → 長名でセル溢れ
                       → <TableCell className="max-w-xs truncate"> など
HorseTable.tsx:100,108 - [x] Pencil / Trash2 に aria-hidden="true" 欠落（OwnerTable と同様）
HorseTable.tsx:57,60   - [x] 頭数の数値表示に font-variant-numeric: tabular-nums 欠落
                         （数値列の桁揃えに使用すべき）
```
